...
class TestApplication:
    ...
    async def test_slot_behaviour(self, bot, mro_slots):
        async with ApplicationBuilder().token(bot.token).build() as app:
            for at in app.__slots__:
                at = f"_Application{at}" if at.startswith(
                    "__") and not at.endswith("__") else at
                assert getattr(app, at,
                               "err") != "err", f"got extra slot '{at}'"
            assert len(mro_slots(app)) == len(set(
                mro_slots(app))), "duplicate slot"
    ...
    @pytest.mark.parametrize("concurrent_updates, expected", [(0, 0), (4, 4),
                                                              (False, 0),
                                                              (True, 256)])
    ...
    def test_custom_context_init(self, bot):
        ...
        application = ApplicationBuilder().token(
            bot.token).context_types(cc).build()
    ...
    async def test_initialize(self, bot, monkeypatch, updater):
        ...
        monkeypatch.setattr(Bot, "initialize",
                            call_after(Bot.initialize, after_initialize_bot))
        monkeypatch.setattr(
            Updater, "initialize",
            call_after(Updater.initialize, after_initialize_updater))
    ...
    async def test_shutdown(self, bot, monkeypatch, updater):
        ...
        monkeypatch.setattr(Bot, "shutdown",
                            call_after(Bot.shutdown, after_bot_shutdown))
        monkeypatch.setattr(
            Updater, "shutdown",
            call_after(Updater.shutdown, after_updater_shutdown))
    ...
    async def test_multiple_inits_and_shutdowns(self, app, monkeypatch):
        ...
        monkeypatch.setattr(app.bot, "initialize",
                            call_after(app.bot.initialize, after_initialize))
        monkeypatch.setattr(app.bot, "shutdown",
                            call_after(app.bot.shutdown, after_shutdown))
    ...
    async def test_start_not_running_after_failure(self, bot, monkeypatch):
        ...
        app = ApplicationBuilder().token(bot.token).job_queue(
            JobQueue()).build()
    ...
    async def test_context_manager(self, monkeypatch, app):
        ...
        monkeypatch.setattr(
            ...
            call_after(Application.initialize, after_initialize))
        monkeypatch.setattr(Application, "shutdown",
                            call_after(Application.shutdown, after_shutdown))
    ...
    async def test_one_context_per_update(self, app):
        ...
        async with app:
            app.add_handler(MessageHandler(filters.Regex("test"), one),
                            group=1)
    ...
    async def test_handler_order_in_group(self, app):
        ...
        app.add_handler(MessageHandler(filters.ALL,
                                       self.callback_set_count(2)))
        app.add_handler(
            MessageHandler(filters.TEXT, self.callback_set_count(3)))
        async with app:
            await app.start()
            await app.update_queue.put(self.message_update)
            await asyncio.sleep(0.05)
    ...
    async def test_add_handlers(self, app):
        ...
        order"""
        msg_handler_set_count = MessageHandler(filters.TEXT,
                                               self.callback_set_count(1))
        msg_handler_inc_count = MessageHandler(filters.PHOTO,
                                               self.callback_increase_count)

        app.add_handler(msg_handler_set_count, 1)
        app.add_handlers((msg_handler_inc_count, msg_handler_inc_count), 1)

        photo_update = make_message_update(
            message=Message(2, None, None, photo=True))
        ...
        async with app:
            ...
            await asyncio.sleep(
                0.05)  # sleep is required otherwise there is random behaviour

            # Test if handler was added to correct group with correct order-
            assert (self.count == 2 and len(app.handlers[1]) == 3
            ...
            with pytest.raises(
                    ValueError,
                    match="The `handlers` argument must be a sequence"):
    ...
    async def test_flow_stop(self, app, bot):
        ...
        update = make_message_update(message=Message(
            1,
            None,
            None,
            None,
        ...
        app.add_handler(
            MessageHandler(
                ...
                self.callback_raise_error(
                    error_message=self.message_update.message.text),
            ))
        app.add_handler(MessageHandler(filters.ALL,
                                       self.callback_set_count(42)),
                        group=1)
    ...
    async def test_error_handler(self, app, block):
        ...
        app.add_handler(
            TypeHandler(object,
                        self.callback_raise_error("TestError"),
                        block=block))
    ...
    @pytest.mark.parametrize(
        "check,expected",
        [(True, True), (None, False), (False, False), ({}, True), ("", True),
         ("check", True)],
    )
    ...
    async def test_non_blocking_handler(self, app):
        ...
        app.add_handler(TypeHandler(object, self.callback_increase_count),
                        group=1)
    ...
    async def test_non_blocking_handler_applicationhandlerstop(
            self, app, recwarn):
        ...
        assert (Path(recwarn[0].filename) == PROJECT_ROOT_PATH / "telegram" /
                "ext" / "_application.py"), "incorrect stacklevel!"

    async def test_non_blocking_no_error_handler(self, app, caplog):
        app.add_handler(
            TypeHandler(object, self.callback_raise_error, block=False))
    ...
    async def test_non_blocking_error_handler(self, app, handler_block):
        ...
        app.add_handler(
            TypeHandler(object, self.callback_raise_error,
                        block=handler_block))
    ...
    @pytest.mark.parametrize(["block", "expected_output"], [(False, 0),
                                                            (True, 5)])
    async def test_default_block_error_handler(self, bot, block,
                                               expected_output):
        ...
        app = Application.builder().token(bot.token).defaults(
            Defaults(block=block)).build()
        async with app:
    ...
    async def test_default_block_handler(self, bot, block, expected_output):
        ...
        async with app:
            app.add_handler(
                TypeHandler(object, self.callback_set_count(5, sleep=0.1)))
            await app.process_update(1)
            await asyncio.sleep(0.05)
            assert self.count == expected_output
    ...
    async def test_nonblocking_handler_raises_and_non_blocking_error_handler_raises(
        ...
        handler = TypeHandler(object,
                              self.callback_raise_error,
                              block=handler_block)
        app.add_handler(handler)
        app.add_error_handler(self.error_handler_raise_error,
                              block=error_handler_block)
        ...
        async with app:
            ...
            with caplog.at_level(logging.ERROR):
                ...
                assert len(caplog.records) == 1
                assert (caplog.records[-1].getMessage().startswith(
                    "An error was raised and an uncaught"))
    ...
    @pytest.mark.parametrize(
        ...
        [
            Message(message_id=1,
                    chat=Chat(id=2, type=None),
                    migrate_from_chat_id=1,
                    date=None),
            Message(message_id=1,
                    chat=Chat(id=1, type=None),
    ...
    def test_migrate_chat_data(self, app, message: "Message", old_chat_id: int,
        ...
        def call(match: str):
            with pytest.raises(ValueError, match=match):
                app.migrate_chat_data(message=message,
        ...
        elif not (isinstance(old_chat_id, int)
                  and isinstance(new_chat_id, int)):
    ...
    async def test_create_task_awaiting_warning(self, app, running, recwarn):
        ...
        async with app:
            ...
            if running:
                ...
                assert len(recwarn) == 1
                assert "won't be automatically awaited" in str(
                    recwarn[0].message)
    ...
    async def test_concurrent_updates(self, bot, concurrent_updates):
        ...
        app = Application.builder().token(
            bot.token).concurrent_updates(concurrent_updates).build()
        events = {
            i: asyncio.Event()
    ...
    async def test_concurrent_updates_done_on_shutdown(self, bot):
        app = Application.builder().token(
            bot.token).concurrent_updates(True).build()
        event = asyncio.Event()
    ...
    def test_run_polling_basic(self, app, monkeypatch):
        ...
        def thread_target():
            ...
            time.sleep(0.05)
            assertions[
                "exception_handling"] = self.received == exception.message
            ...
            assertions[
                "job_queue_not_running"] = not app.job_queue.scheduler.running
    ...
    def test_run_polling_post_init(self, bot, monkeypatch):
        ...
        app = Application.builder().token(
            bot.token).post_init(post_init).build()
        monkeypatch.setattr(app.bot, "get_updates", get_updates)
        monkeypatch.setattr(
            app, "initialize",
            call_after(app.initialize, lambda _: events.append("init")))
        monkeypatch.setattr(
            app.updater,
            "start_polling",
        ...
        app.run_polling(drop_pending_updates=True, close_loop=False)
        thread.join()
        assert events == ["init", "post_init",
                          "start_polling"], "Wrong order of events detected!"
    ...
    def test_run_polling_post_shutdown(self, bot, monkeypatch):
        ...
        app = Application.builder().token(
            bot.token).post_shutdown(post_shutdown).build()
        monkeypatch.setattr(app.bot, "get_updates", get_updates)
        monkeypatch.setattr(
            app, "shutdown",
            call_after(app.shutdown, lambda _: events.append("shutdown")))
        monkeypatch.setattr(
            app.updater,
            "shutdown",
            call_after(app.updater.shutdown,
                       lambda _: events.append("updater.shutdown")),
        )
    ...
    def test_run_polling_parameters_passing(self, app, monkeypatch):
        ...
        assert set(self.received.keys()) == set(
            updater_signature.parameters.keys())
        for name, param in updater_signature.parameters.items():
            if name == "error_callback":
        ...
        expected = {
            name: name
            for name in updater_signature.parameters
            if name != "error_callback"
        }
        thread = Thread(target=thread_target)
        thread.start()
    ...
    def test_run_webhook_basic(self, app, monkeypatch):
        ...
        def thread_target():
            ...
            loop.run_until_complete(
                send_webhook_message(ip, port, self.message_update.to_json(),
                                     "TOKEN"))
            loop.close()
            time.sleep(0.05)
            assertions["getting_updates"] = self.count == 42
    ...
    def test_run_webhook_post_init(self, bot, monkeypatch):
        ...
        monkeypatch.setattr(
            app.updater,
            "start_webhook",
            call_after(app.updater.start_webhook,
                       lambda _: events.append("start_webhook")),
        )
        ...
        thread.join()
        assert events == ["init", "post_init",
                          "start_webhook"], "Wrong order of events detected!"
    ...
    def test_run_webhook_parameters_passing(self, bot, monkeypatch):
        ...
        assert set(self.received.keys()) == set(
            updater_signature.parameters.keys()) - {"self"}
        for name, param in updater_signature.parameters.items():
            if name == "self":
                continue
            assert self.received[name] == param.default
        ...
        expected = {
            name: name
            for name in updater_signature.parameters if name != "self"
        }
        thread = Thread(target=thread_target)
        thread.start()
    ...
    def test_run_without_updater(self, bot):
        ...
        with pytest.raises(
                RuntimeError,
                match="only available if the application has an Updater"):
            app.run_webhook()
    ...
    def test_run_error_in_application(self, bot, monkeypatch, method):
        ...
        monkeypatch.setattr(
            Updater, "shutdown",
            call_after(Updater.shutdown, after_shutdown("updater")))
        app = ApplicationBuilder().token(bot.token).build()
    ...
    @pytest.mark.parametrize("method", ["start_polling", "start_webhook"])
    def test_run_stop_signal_warning_windows(self, bot, method, recwarn,
                                             monkeypatch):
        ...
        for record in recwarn:
            ...
            if str(record.message).startswith(
                    "Could not add signal handlers for the stop"):
    ...
    def test_signal_handlers(self, app, monkeypatch):
        ...
        if platform.system() == "Windows":
            ...
            assert received_signals == [
                signal.SIGINT, signal.SIGTERM, signal.SIGABRT
            ]

        received_signals.clear()
        loop.create_task(abort_app())
        app.run_webhook(port=49152,
